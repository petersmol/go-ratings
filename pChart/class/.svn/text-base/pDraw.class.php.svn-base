<?php
	...
	include_once ("/home/smol/www/petersmol.ru/go/include/rating_functions.php");
 /*
       ...
       while(!$Found)
        {
         ...
         $Scaled10Factor = $Scaled10Factor * 100; // * 10; Smol dirty hack
        }
...
/*       if ( $Mode == AXIS_FORMAT_GO && $Format == NULL )
        {
             ...
             for($i=0;$i<=$Rows;$i++)
              {
               ...
               $Label = $this->scaleFormat($Value,AXIS_FORMAT_GO,$Decimals);
   ...
   function scaleFormat($Value,$Mode=NULL,$Format=NULL,$Unit=NULL)
    {
     ...
     if ( $Mode == AXIS_FORMAT_GO )
      { return elo2kyu($Value); }
